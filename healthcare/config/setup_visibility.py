...
def setup_4geeks_health_visibility():
    ...
    # Roles estándar
    standard_roles = [
        "Healthcare Practitioner", "Accounts User", "Stock User",
        "HR User", "Buying User", "Sales User"
    ]

    for role in standard_roles:
        for module in modules_to_hide:
            if not frappe.db.exists("Module Profile", {"module": module, "role": role}):
                frappe.get_doc({
